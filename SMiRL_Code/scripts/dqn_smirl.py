...
"""
import sys
api_key="poDlVCYEV8mDLEEQMHHIsfkn1"
project_name="SMiRL_Test2022"
workspace="glenb"

try:
    from comet_ml import Experiment
except:
    print("Cannot import comet_ml")
...
def get_env(variant):
    ...
    if (variant["env"] == "Tetris"):
        ...
        env = VizDoomEnv(config_path=BASE_CODE_DIR + "SMiRL_Code/" + env_wargs["doom_scenario"],
                         **env_wargs)
    else: 
        import gym
        import crafter
...
def add_wrappers(env, variant, device=0, eval=False, network=None):
    ...
    for wrapper in variant["wrappers"]:
        ...
        elif "strip_info" in wrapper:
            from surprise.wrappers.obsresize import StripInfoWrapper
            env = StripInfoWrapper(env=env)
...
def experiment(doodad_config, variant):
    ...
    if (variant["log_comet"]):
        try:
            comet_logger = Experiment(api_key=api_key,
            ...
            print(comet_logger)
            print(variant['comet_key'] + " Debugging here")
...
#     base_env2 = RenderingObservationWrapper(base_env2)
    ...
    if "prob_random_action" in variant:
        ...
        expl_policy = PolicyWrappedWithExplorationStrategy(
            EpsilonGreedy(expl_env.action_space, prob_random_action=0.6, prob_end=0.05),
            eval_policy,
        )
