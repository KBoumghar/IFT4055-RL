...
def display_gif(images, logdir, fps=10, max_outputs=8, counter=0):
    ...
    clip = mpy.ImageSequenceClip(list(images), fps=fps, durations=None, with_mask=False)
    ...
    if cl is not None:
        ...
        print("No comet logger")

    # matplotlib.pyplot.imshow() #Shows what the pixels are (useful for making this work)
...
class TorchBatchRLRenderAlgorithm(TorchBatchRLAlgorithm):
    ...
    def render_video(self, tag, counter):
        ...
        if ("vae_reconstruction" in path[0]['env_infos'][0]):
            ...
            ##TODO : Check why this is not right for vae_reconstruction (is it just a crafter thing?)
            #np.transpose(video, (0, 1, 3, 2, 4))
            display_gif(images=video, logdir=logger.get_snapshot_dir() + "/" + tag + "_reconstruction", fps=15,
                        counter=counter)
