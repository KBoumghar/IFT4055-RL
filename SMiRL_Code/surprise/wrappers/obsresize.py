...
class StripInfoWrapper(gym.Env):
    ...
    def step(self, action):
        # Take Action
        obs, env_rew, envdone, _ = self._env.step(action)
        return obs, env_rew, envdone, {}
...
class ResizeObservationWrapper(gym.Env):
    ...
    def render(self, mode=None):
        return self._env.render(mode=None)
...
class RenderingObservationWrapper(gym.Env):
    ...
    def step(self, action):
        ...
        obs, env_rew, envdone, info = self._env.step(action)
        test = self.unwrapped.render("rgb_array")
        info["rendering"] = self._env.render(mode="rgb_array")

        if (self._resize is not None):
            try:
                info["rendering"] = cv2.resize(info["rendering"], dsize=tuple(self._resize[:2]),
...
class ObsHistoryWrapper(gym.Env):
    ...
    def reset(self):
        ...
        self.obs_hist = collections.deque(
            [np.zeros(shape=self.observation_space_old.low.shape) for _ in range(self._history_length)])
